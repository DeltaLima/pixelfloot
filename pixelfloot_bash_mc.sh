...
PPMFILE="$2.ppm"
HEXPPM="$2-mc.hexppm.$COLORSHIFT"
PIXLIST="$2-mc.pixlist.$COLORSHIFT"
FNAME="$2"
ALPHACOLOR="$3"
...
gen_pixmap() {
	...
	do
		...
		then
			...
			do
				REDindex=$((x*3-COLORSHIFT))
				GREENindex=$((x*3+1-COLORSHIFT))
				BLUEindex=$((x*3+2-COLORSHIFT))
...
#~ $(cat $PIXLIST | shuf)"
	...
	#~ done
	LOL[1]="$(cat $FNAME-mc.pixlist.0 | shuf)"
	LOL[2]="$(cat $FNAME-mc.pixlist.1 | shuf )"
